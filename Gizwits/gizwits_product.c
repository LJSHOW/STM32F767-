  ...
  for(i=0; i<info->num; i++)
  {
    switch(info->event[i])
    {
      ...
      case  EVENT_fan:
        ...
        GIZWITS_LOG("Evt: EVENT_valuefan %d \n", currentDataPoint.valuefan);
        if(0x01 == currentDataPoint.valuefan)
        {
          //user handle
			  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_3,GPIO_PIN_SET);//LED1亮
        }
        else
        {
          //user handle  
             HAL_GPIO_WritePin(GPIOB,GPIO_PIN_3,GPIO_PIN_RESET);//LED1灭	 					
        }
        break;
      ...
      case WIFI_DISCON_ROUTER:
        break;
      case WIFI_CON_M2M: wifi_sta=1;//wifi设备已连接
        break;
      case WIFI_DISCON_M2M: wifi_sta=0;//wifi设备断开
        break;
      case WIFI_RSSI:
        GIZWITS_LOG("RSSI %d\n", wifiData->rssi);
        break;
...
int32_t uartWrite(uint8_t *buf, uint32_t len)
{
    ...
    for(i=0; i<len; i++)
    {
        ...
        {
          //实现串口发送函数,将0x55发送到模组
		 HAL_UART_Transmit(&UART3_Handler,data55,1,1000);
         while(__HAL_UART_GET_FLAG(&UART3_Handler,UART_FLAG_TXE)==RESET);
#ifdef PROTOCOL_DEBUG
