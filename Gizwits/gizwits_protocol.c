...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagfan)
    {
        info->event[info->num] = EVENT_fan;
        info->num++;
        dataPoints->valuefan = gizStandardDecompressionValue(fan_BYTEOFFSET,fan_BITOFFSET,fan_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuehumidity_alarm != cur->valuehumidity_alarm)
    {
        GIZWITS_LOG("valuehumidity_alarm Changed\n");
        ret = 1;
    }
    if(last->valueMQ_7_alarm != cur->valueMQ_7_alarm)
    {
        GIZWITS_LOG("valueMQ_7_alarm Changed\n");
        ret = 1;
    }
    if(last->valueMG811_alarm != cur->valueMG811_alarm)
    {
        GIZWITS_LOG("valueMG811_alarm Changed\n");
        ret = 1;
    }
    if(last->valueGP2Y1014AU_ALARM != cur->valueGP2Y1014AU_ALARM)
    {
        GIZWITS_LOG("valueGP2Y1014AU_ALARM Changed\n");
        ret = 1;
    }
    ...
    if(last->valueMQ_7 != cur->valueMQ_7)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMQ_7 Changed\n");
            ret = 1;
        }
    }
    if(last->valueMG811 != cur->valueMG811)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMG811 Changed\n");
            ret = 1;
        }
    }
    if(last->valueGP2Y1014AU != cur->valueGP2Y1014AU)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueGP2Y1014AU Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(fan_BYTEOFFSET,fan_BITOFFSET,fan_LEN,(uint8_t *)devStatusPtr,dataPoints->valuefan);
    ...
    gizStandardCompressValue(humidity_alarm_BYTEOFFSET,humidity_alarm_BITOFFSET,humidity_alarm_LEN,(uint8_t *)devStatusPtr,dataPoints->valuehumidity_alarm);
    gizStandardCompressValue(MQ_7_alarm_BYTEOFFSET,MQ_7_alarm_BITOFFSET,MQ_7_alarm_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMQ_7_alarm);
    gizStandardCompressValue(MG811_alarm_BYTEOFFSET,MG811_alarm_BITOFFSET,MG811_alarm_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMG811_alarm);
    gizStandardCompressValue(GP2Y1014AU_ALARM_BYTEOFFSET,GP2Y1014AU_ALARM_BITOFFSET,GP2Y1014AU_ALARM_LEN,(uint8_t *)devStatusPtr,dataPoints->valueGP2Y1014AU_ALARM);
    ...
    devStatusPtr->valueMQ_7 = exchangeBytes(gizY2X(MQ_7_RATIO,  MQ_7_ADDITION, dataPoints->valueMQ_7)); 
    devStatusPtr->valueMG811 = exchangeBytes(gizY2X(MG811_RATIO,  MG811_ADDITION, dataPoints->valueMG811)); 
    devStatusPtr->valueGP2Y1014AU = exchangeBytes(gizY2X(GP2Y1014AU_RATIO,  GP2Y1014AU_ADDITION, dataPoints->valueGP2Y1014AU)); 
