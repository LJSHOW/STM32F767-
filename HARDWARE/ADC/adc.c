...
{ 
    ...
    ADC1_Handler.Init.NbrOfConversion=1;                         //1个转换在规则序列中 也就是只转换规则序列1 其它数据不变
    ADC1_Handler.Init.DiscontinuousConvMode=ENABLE;             //禁止不连续采样模式
    ADC1_Handler.Init.NbrOfDiscConversion=4;                     //不连续采样通道数为0
...
void HAL_ADC_MspInit(ADC_HandleTypeDef* hadc)
{
	...
	GPIO_Initure.Pin=GPIO_PIN_7;            //PA1
	HAL_GPIO_Init(GPIOA,&GPIO_Initure);
}
